...
export default function Home() {
    ...
    useEffect(() => {
        ...
        async function getUser() {
            try {
                const response =  await axios.get(`http://localhost:5500/user/${localStorage.getItem('id')}`, {
                    headers,
                });
    ...
    const OnFinish = async (values: any) => {
        ...
        const studentInfo = await axios.get(`http://localhost:5500/user/name/${values.student}`, {
            headers,
        });
        console.log(studentInfo)

        try {
            await axios.post(`http://localhost:5500/transfer`, {
                value: values.coin,
